...
function App() {
  ...
  // Settings state
  const [activeTab, setActiveTab] = useState('analyze') // 'analyze' | 'settings'
  ...
  const fetchDepartments = async () => {
    try {
      setSettingsError('')
      const res = await fetch(`${backend}/settings/departments`)
      if (!res.ok) throw new Error(await res.text())
      const list = await res.json()
      setDepartments(Array.isArray(list) ? list : [])
    } catch (e) {
      setSettingsError('Failed to load departments.');
    }
  }
  ...
  const addDepartment = async () => {
    ...
    try {
      ...
      setSettingsError('Failed to add department.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const deleteDepartment = async (name) => {
    try {
      ...
      const res = await fetch(`${backend}/settings/departments/${encoded}`, {
        method: 'DELETE',
      })
      if (!res.ok) throw new Error(await res.text())
      const list = await res.json()
      ...
      setSettingsError('Failed to delete department.')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="mx-auto max-w-6xl px-4 -mt-8">
        <div className="flex items-center gap-2">
          <button
            className={`px-4 py-2 rounded-t-xl border-b-2 ${activeTab === 'analyze' ? 'border-blue-500 text-white' : 'border-transparent text-slate-400 hover:text-white'}`}
            onClick={() => setActiveTab('analyze')}
          >
            Analyze
          </button>
          <button
            className={`px-4 py-2 rounded-t-xl border-b-2 ${activeTab === 'settings' ? 'border-blue-500 text-white' : 'border-transparent text-slate-400 hover:text-white'}`}
        ...
        {activeTab === 'analyze' ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 md:p-6 shadow-xl">
              ...
              {/* Configured departments (for user visibility) */}
              <div className="mb-4">
                <div className="text-xs uppercase tracking-wider text-slate-400 mb-2">Configured Departments (source set)</div>
                <div className="flex flex-wrap gap-2">
                  {departments && departments.length > 0 ? (
                    departments.map((d, i) => <Tag key={i} label={d} />)
                  ) : (
                    <span className="text-slate-400 text-sm">No departments configured</span>
                  )}
                </div>
              </div>
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 md:p-6 shadow-xl">
            ...
            <p className="text-slate-400 text-sm mt-1">Manage the list of departments used by the AI when mapping circulars.</p>

            <div className="mt-5">
              <label className="block text-sm mb-2 text-slate-300">Add a department</label>
              <div className="flex items-center gap-2">
                <input
                  value={newDept}
                  onChange={(e) => setNewDept(e.target.value)}
                  placeholder="e.g. Cards & ATM Network"
                  className="flex-1 rounded-lg bg-slate-950/60 border border-slate-800/80 focus:border-blue-500/60 outline-none px-3 py-2 text-slate-100 placeholder-slate-500"
                />
                <button
                  onClick={addDepartment}
                  disabled={saving || !newDept.trim()}
                  className="rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-60 disabled:cursor-not-allowed px-4 py-2 font-semibold"
                >
                  Add
                </button>
              </div>
            ...
            <div className="mt-6">
              <div className="text-sm font-medium mb-2">Current departments ({departments.length})</div>
              {departments.length === 0 ? (
                <div className="text-slate-400 text-sm">No departments configured.</div>
              ) : (
                <ul className="divide-y divide-slate-800 rounded-lg border border-slate-800 overflow-hidden">
                  {departments.map((d, i) => (
                    <li key={i} className="flex items-center justify-between px-3 py-2 bg-slate-950/40">
                      <span className="text-slate-200 text-sm">{d}</span>
                      <button
                        onClick={() => deleteDepartment(d)}
                        disabled={saving}
                        className="text-xs text-red-300 hover:text-red-200 px-2 py-1 border border-red-800/60 rounded-md"
                      >
                        Delete
                      </button>
                    </li>
            ...
            <div className="mt-6 text-xs text-slate-400">
              Changes are saved on the server. Re-run an analysis to see the AI choose from your updated list.
            </div>
          </div>
        )}
      </div>
